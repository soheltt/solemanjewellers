...
function loadSettings() {
    ...
    if (settings) {
        $('#basePrice24ct').value = settings.basePrice24ct || 122000;
...
function fetchLivePrice() {
    ...
    const currentPrice24ct = getVal('basePrice24ct');
...
function calculateProfit(returnDetails = false) {
  ...
  let priceRatio = { hallmark: 0.98, kdm: 0.82, bengali: 0.81, desi_bengali: 0.71 }[details.quality];
...
function calculateAnaPrice() {
    const weightInGrams = getVal('gramInput');
    const pricePer10g = getVal('anaPricePer10g');
    const quality = $('#anaGoldQuality').value;

    if (weightInGrams <= 0 || pricePer10g <= 0) {
        $('#anaResult').innerHTML = 'অনুগ্রহ করে সঠিক ওজন এবং দাম লিখুন।';
        return;
    }

    let priceRatio = { hallmark: 0.98, kdm: 0.82, bengali: 0.81, '24ct': 1.0, desi_bengali: 0.71 }[quality];
    
    const finalPrice = toFixedTrim((pricePer10g * priceRatio / 10) * weightInGrams);

    $('#anaResult').innerHTML = `<strong style="font-size: 1.2em;">মোট মূল্য: <span style="color: var(--primary-dark)">₹${finalPrice.toLocaleString('en-IN')}</span></strong>`;
}
...
function saveAndPrintBill() {
    ...
    billWindow.document.write(`
        ...
        <style>
            ...
            <div class="header">
                <h2>সোলেমান জুয়েলার্স</h2> <p>ঠিকানা: ভাদুরিয়াপাড়া বাজার | মোবাইল: 8967575884</p> <p>GSTIN: 19BMZPR4273E1Z9 | BIS Hallmark: HM/C-5191720314</p>
            </div>
            <div class="meta">
...
function resetProfit() {
  ...
  $('#basePrice24ct').value = '122000'; 
  $('#makingChargePerGram').value = '500';
  $('#goldQuality').value = 'hallmark';
  calculateProfit();
  updateAllAnaDisplays();
}
function resetExchange(){
  $('#newWeight').value='0'; 
  $('#newPrice11').value = '122000';
  $('#labourType').value = 'pergram';
  $('#labourValue').value='500'; 
  $('#oldWeight').value='0';
  $('#oldPrice11').value = '122000';
  $('#deduct').value = '35';
...
function resetSilverExchange(){
  ...
  $('#newPrice11Silver').value='1010'; $('#oldPrice11Silver').value='1010';
...
function resetAnaConverter() {
    $('#anaInput').value = '';
    $('#gramInput').value = '';
    $('#anaPricePer10g').value = '122000';
    $('#anaGoldQuality').value = 'hallmark';
    calculateAnaPrice();
}
...
function otn_calculateOldToNew() {
    ...
    let pureGoldEq;
    if (oldType === 'bengali' && newType === 'kdm') pureGoldEq = oldWeight * 63 / 82;
    else if (oldType === 'bengali' && newType === 'bengali') pureGoldEq = oldWeight * 65 / 75;
    else {
        const oldPurityFactors = { hallmark: 85/92, bengali: 63/92, kdm: 82/92, desi_bengali: 71/92 };
        pureGoldEq = oldWeight * oldPurityFactors[oldType];
    }
    otn_calculationResult.pureGoldEq = pureGoldEq;

    const deductionInGrams = oldWeight - pureGoldEq;
    const newGoldPriceRatio = { hallmark: 0.98, bengali: 0.81, kdm: 0.82, desi_bengali: 0.71 };
    ...
    if (deficitWeight < -0.001) { // Case 1: EXCESS gold
        ...
        const pricePerGramNew = (price24ct * newGoldPriceRatio[newType]) / 10;
        ...
        const pricePerGramNew = (price24ct * (newGoldPriceRatio[newType] || 0.98)) / 10;
...
function otn_toggleDetails(button) {
    ...
    if (details.style.display === 'none') {
        ...
        button.textContent = 'সম্পূর্ণ হিসাব লুকান';
    } else {
...
function otn_resetOldToNew() {
    ...
    $('#otn_NewType').value = 'hallmark';
    $('#otn_BasePrice24ct').value = '122000';
    ...
    return `
        <div style="font-family: 'Noto Sans Bengali', sans-serif; color: #0f172a; padding: 15px;">
            <div style="text-align: center; border-bottom: 2px solid #0ea5e9; padding-bottom: 10px; margin-bottom: 20px;">
                ...
                <p style="margin: 5px 0; font-size: 12px;">GSTIN: 19BMZPR4273E1Z9 | BIS Hallmark: HM/C-5191720314</p>
...
window.onload = function() {
  ...
  setupWeightConversion('gramInput', 'anaInput');
  ...
  document.querySelectorAll('#anaConverter input, #anaConverter select').forEach(input => {
      input.addEventListener('input', calculateAnaPrice);
  });
